...
{
    public async Task<EvaluationInput?> InputAsync(Guid id, CancellationToken ct)
    {
        var r = await context.AttendanceRecords.IgnoreQueryFilters().AsNoTracking().Include(r => r.Shift).SingleOrDefaultAsync(r => r.Id == id && !r.IsDeleted, ct);
        return r is null ? null : new(r, r.Shift.StartTime, r.Shift.ShiftName);
    }
    ...
    public async Task<AttendanceEvaluation?> PreviousAsync(Guid employee, DateOnly date, CancellationToken ct)
    {
        var raw = await context.AttendanceEvaluations.IgnoreQueryFilters().AsNoTracking().Where(e => e.EmployeeId == employee && e.AttendanceDate < date && e.ReasonCode != "HistoricalOrderReviewRequired").OrderByDescending(e => e.AttendanceDate).FirstOrDefaultAsync(ct);
        var revision = await context.AttendanceEvaluationRevisions.IgnoreQueryFilters().AsNoTracking().Where(e => e.EmployeeId == employee && e.AttendanceDate < date).OrderByDescending(e => e.AttendanceDate).ThenByDescending(e => e.Version).FirstOrDefaultAsync(ct);
        return revision is not null && (raw is null || revision.AttendanceDate >= raw.AttendanceDate) ? EvaluationSnapshots.Read(revision) : raw;
    }

    public async Task<bool> LaterAsync(Guid employee, DateOnly date, CancellationToken ct) => await context.AttendanceEvaluations.IgnoreQueryFilters().AnyAsync(e => e.EmployeeId == employee && e.AttendanceDate > date, ct) || await context.AttendanceEvaluationRevisions.AnyAsync(e => e.EmployeeId == employee && e.AttendanceDate > date, ct);
    public async Task AddAsync(AttendanceEvaluation evaluation, AttendancePenaltyEvent? penalty, CancellationToken ct)
    {
        await context.AddAsync(evaluation, ct);
        if (penalty is not null)
            await context.AddAsync(penalty, ct);
    }
    ...
    private IQueryable<AttendanceEvaluation> Filter(EvaluationQuery q)
    {
        var v = History;
        if (q.CompanyId is { } c)
            v = v.Where(e => e.CompanyId == c);
        if (q.EmployeeId is { } id)
            v = v.Where(e => e.EmployeeId == id);
        if (q.BranchId is { } b)
            v = v.Where(e => e.AttendanceRecord.WorkLocation.BranchId == b);
        if (q.FromDate is { } from)
            v = v.Where(e => e.AttendanceDate >= from);
        if (q.ToDate is { } to)
            v = v.Where(e => e.AttendanceDate <= to);
        if (q.LateOnly)
            v = v.Where(e => e.IsLate);
        if (q.PenaltyOnly)
            v = v.Where(e => e.PenaltyTriggered);
        return v;
    }
    ...
    private Expression<Func<AttendancePenaltyEvent, PenaltyResponse>> PenaltyProjection => p => new()
    {
        PolicyImpactReviewRequired = context.AttendanceEvaluationRevisions.Any(v => v.RelatedPenaltyEventId == p.Id && v.PolicyImpactReviewRequired),
        CompanyId = p.CompanyId,
        Id = p.Id,
        EmployeeId = p.EmployeeId,
        AttendanceRecordId = p.AttendanceRecordId,
        AttendanceEvaluationId = p.AttendanceEvaluationId,
        AttendancePolicyId = p.AttendancePolicyId,
        EmployeeCode = p.AttendanceEvaluation.Employee.EmployeeCode,
        EmployeeName = p.AttendanceEvaluation.Employee.FirstName + (p.AttendanceEvaluation.Employee.LastName == null ? "" : " " + p.AttendanceEvaluation.Employee.LastName),
        PenaltyDate = p.PenaltyDate,
        LateMinutes = p.AttendanceEvaluation.LateMinutes,
        ConsecutiveLateCount = p.AttendanceEvaluation.ConsecutiveLateCount,
    ...
    {
        var ids = Filter(q).Select(e => e.Id);
        return await context.AttendancePenaltyEvents.IgnoreQueryFilters().AsNoTracking().Where(p => !p.IsDeleted && ids.Contains(p.AttendanceEvaluationId)).OrderByDescending(p => p.PenaltyDate).ThenBy(p => p.Id).Skip(q.Skip).Take(q.Take).Select(PenaltyProjection).ToListAsync(ct);
    }
