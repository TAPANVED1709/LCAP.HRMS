...
namespace LCAP.HRMS.Day04Harness;
...
public static class HarnessProgram
{
    public static async Task Main(string[] args)
    {
        ...
        if (!System.Text.RegularExpressions.Regex.IsMatch(database, "^LCAP_HRMS_Day04_[A-Za-z0-9_]+$"))
            throw new ArgumentException("Only isolated Day04 test databases are accepted.");
        ...
        var token = new JwtSecurityTokenHandler().WriteToken(new JwtSecurityToken(
            ...
            claims: [new Claim("sub", "day04-integration-tester"), new Claim("role", "SuperAdmin")],
        ...
        var output = Path.Combine(root, "artifacts", "day04");
        ...
        builder.WebHost.UseUrls("http://127.0.0.1:5094");
        ...
        {
            var claims = new List<Claim> {new("sub", "day04-role-test"),new("role",identity.Role)};
        ...
        app.MapGet("/test/time", () => factory.Clock.GetUtcNow().ToUnixTimeMilliseconds());
        app.MapPost("/test/clock", (TestClockRequest request) => {factory.Clock.Set(request.Utc);return Results.NoContent();});
        app.MapPost("/test/evaluate/{id:guid}", async (Guid id) => {using var scope=factory.Services.CreateScope();await scope.ServiceProvider.GetRequiredService<LCAP.HRMS.Application.AttendancePolicies.IAttendanceEvaluationService>().EvaluateAsync(id,default);return Results.NoContent();});
        app.UseStaticFiles();
        ...
        app.MapGet("/test/attendance", async context => {
            ...
            var shim = "<script>navigator.geolocation.getCurrentPosition=(ok,fail,options)=>fetch('/test/time').then(r=>r.json()).then(timestamp=>ok({coords:{latitude:0,longitude:0,accuracy:10},timestamp}));</script>";
            context.Response.ContentType="text/html";
            await context.Response.WriteAsync(html.Replace("<head>","<head>"+shim));
        });
        app.MapFallbackToFile("index.html");
        Console.WriteLine($"Day04 SQL integration host ready at http://127.0.0.1:5094; database={database}");
        await app.RunAsync();
...
{
    public const string Issuer = "https://day04.test.invalid";
...
public sealed record TestClockRequest(DateTimeOffset Utc);
public sealed class TestClock:TimeProvider {private long ticks=DateTimeOffset.UtcNow.UtcTicks;public void Set(DateTimeOffset utc)=>Interlocked.Exchange(ref ticks,utc.UtcTicks);public override DateTimeOffset GetUtcNow()=>new(Interlocked.Read(ref ticks),TimeSpan.Zero);}
